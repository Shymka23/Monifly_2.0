...
const CategoryPieChart = memo(function CategoryPieChart({
  ...
  const {
    ...
    getPeriodCategoryExpenseBreakdown: state.getPeriodCategoryExpenseBreakdown,
  ...
  const chartData = useMemo(() => {
    if (!isClient) return [];
    const data = getPeriodCategoryExpenseBreakdown();
    ...
    if (data.length > 5) {
      ...
      const otherEntry = {
        ...
        originalName: "Other" as TransactionCategory,
      };
      return [...top5, otherEntry];
    }
    return data;
  }, [isClient, getPeriodCategoryExpenseBreakdown]);
    ...
    return (
      <Card className="flex flex-col shadow-lg min-h-[350px]">
        <CardHeader>
          ...
          <CardDescription>
            {t("charts.periodInCurrency", {
              period: selectedPeriodLabel,
              currency: primaryDisplayCurrency,
            })}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex-1 items-center justify-center text-center text-muted-foreground p-4">
          <div>
          ...
          <ChartContainer config={{}} className="w-full h-full">
            <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
              {chartData.map((entry, index) => {
                ...
                if (innerRadius < 10) return null; // Avoid rings collapsing in the center
